...
|文件名|推荐填写的控制命令|
|-|-|
|`WZRY.mynode.临时初始化.txt`    | 控制脚本功能：运行时间、[礼包](/wzry.doc/guide/libao/)等功能的开启关闭。|
|`WZRY.mynode.对战前插入.txt`    | 控制对局过程：[对战模式(快速对战、标准对战、TOUCH模式)](/wzry.doc/guide/duizhanmoshi/)、对战分路、对战英雄、[自动选择熟练度最低的英雄](/wzry.doc/guide/shuliandu/)。|
|`WZRY.mynode.重新设置英雄.txt`  | 覆盖上面的设定，选择指定英雄|
...
## 示例
###  设定游戏的运行时间
仅在每天的5点到17点执行脚本，在`WZRY.mynode.临时初始化.txt` 中填写
```
self.对战时间=[5,17]
```

每天只对战5局，在`WZRY.mynode.临时初始化.txt` 中填写
```
...
###   指定对战的分路和英雄
在`WZRY.mynode.重新设置英雄.txt`中填写下面内容
