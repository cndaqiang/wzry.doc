...
* 则在`wzry.py`所在的文件夹,创建`WZRY.图片更新.txt`文件
...
??? Note "为何要更新图片?"
    ...
    在[组队](zudui.md)的时候，不同账户的头像不同, 也可以将[房主头像](zudui.md#更新图片资源)定义在`WZRY.图片更新.txt`中.
...
## 截取新图片的流程
当发现王者活动更新了`大厅对战图标`(或者任何图片资源), 而本文档又没有及时提供[更新资源](upfig.md),你就可以自己动手更新了.具体流程如下图
